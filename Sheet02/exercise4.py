...
def get_model() -> FeedforwardNet:
    ...
    # Load weights and biases. Note that the weights have to
    # be transposed in order to use them and an additional
    # axis has to be added to the biases
    (W1, b1), (W2, b2) = load_parameters()

    # Create network with one hidden layer of size 100
    # and an output layer of size 10.
    model = FeedforwardNet([784, 100, 10])

    # Set the weights and the bias of the hidden layer
    model.set_weights(W1.T, 0)
    model.set_bias(b1[:, np.newaxis], 0)

    # Set the weights and the bias of the output layer
    model.set_weights(W2.T, 1)
    model.set_bias(b2[:, np.newaxis], 1)
