...
def get_parameters(model: tf.keras.Sequential) -> list[tuple[ArrayLike, ArrayLike]]:
    """
    Get all weights and biases of a FNN.

    Parameters
    ----------
    model : tf.keras.Sequential
        Network to extract the parameters from.

    Returns
    -------
    list[tuple[ArrayLike, ArrayLike]]
        Layerwise list of weight matrix and bias tuples.
    """
...
def save_parameters():
    """
    Train a FNN with the MNIST dataset and save the weights.
    """
    ...
    (W1, b1), (W2, b2) = get_parameters(model)
    np.savez('MNIST_params', W1=W1, W2=W2, b1=b1, b2=b2)
...
if __name__ == '__main__':
    ...
    # Generate the network with the get_model function
    model = get_model(hidden_neurons, 'relu', 'linear')
