...
class Sequential:
    ...
    def train(self, train_input: np.ndarray, train_labels: np.ndarray,
              batch_size: int = 1, epochs: int = 10, verbose: bool=True) -> None:
        """
        ...
        for e in range(epochs):
            ...
            for j in range(n_batches):
                if verbose:
                    print(f'Epoch {e+1:>{len(str(epochs))}d}/{epochs}, Batch {j+1:>{len(str(n_batches))}d}/{n_batches}', end='\r')
